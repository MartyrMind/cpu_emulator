...
class TestFlags:
    ...
    @allure.description("Проверяет корректность операций установки и чтения значений флагов")
    ...
    @allure.description("Проверяет корректность обработки ошибок при работе с несуществующими флагами")
    ...
    @pytest.mark.parametrize("value, expected_z, expected_s, expected_p, description", [
    ...
    ], ids=["zero", "negative", "positive", "max_positive", "all_bits", "odd_parity", "even_parity"])
    @allure.title("Базовое обновление флагов: {description}")
    @allure.description("Проверяет правильность установки флагов Z, S, P при базовом обновлении")
    ...
    @pytest.mark.parametrize("value, expected_parity, description", [
        (0xFF, 1, "8 единиц"),
        (0x0F, 1, "4 единицы"),
        (0x07, 0, "3 единицы"),
        (0x01, 0, "1 единица"),
    ...
    ], ids=["8bits", "4bits", "3bits", "1bit", "0bits", "2bits", "3bits_alt", "4bits_alt"])
    @allure.title("Вычисление четности: {description}")
    @allure.description("Проверяет правильность вычисления флага четности для различных значений")
    def test_parity_calculation(self, value, expected_parity, description):
        """Тест вычисления четности"""
        flags = Flags()
        assert flags._calculate_parity(value) == expected_parity, f"Failed for {description}"

    @pytest.mark.parametrize("a, b, operation, expected_z, expected_s, expected_c, expected_o, description", [
        # Тесты сложения
    ...
    ], ids=["add_normal", "add_carry", "add_overflow", "add_negative", 
            "sub_borrow", "sub_normal", "sub_overflow", "sub_equal",
            "cmp_equal", "cmp_less", "cmp_greater"])
    @allure.title("Арифметическое обновление флагов: {description}")
    @allure.description("Проверяет правильность обновления флагов при арифметических операциях и сравнении")
    def test_arithmetic_update(self, a, b, operation, expected_z, expected_s, expected_c, expected_o, description):
    ...
    @allure.description("Проверяет правильность обновления флагов при логических операциях")
    ...
    @allure.description("Проверяет правильность сброса всех флагов в исходное состояние")
    def test_reset(self):
    ...
    @pytest.mark.parametrize("result, full_result, expected_z, expected_c, description", [
    ...
    @allure.description("Проверяет правильность обновления флагов при операции умножения")
    def test_multiplication_update(self, result, full_result, expected_z, expected_c, description):
    ...
    @pytest.mark.parametrize("quotient, expected_z, description", [
    ...
    @pytest.mark.parametrize("original, count, result, expected_z, expected_c, description", [
    ...
    def test_shift_left_update(self, original, count, result, expected_z, expected_c, description):
    ...
    def test_shift_right_update(self, original, count, result, expected_z, expected_c, description):
    ...
    @pytest.mark.parametrize("result, carry_out, expected_z, expected_c, description", [
    ...
    @allure.description("Проверяет правильность обновления флагов при операциях поворота")
    def test_rotate_update(self, result, carry_out, expected_z, expected_c, description):
