...
class TestALU:
    ...
    @allure.description("Проверяет правильность инициализации арифметическо-логического устройства")
    ...
    @pytest.mark.parametrize("a, b, expected_result, expected_z, expected_s, expected_c, expected_o, description", [
    ...
    @allure.description("Проверяет правильность выполнения операции сложения и обновления флагов")
    def test_add(self, alu_setup, a, b, expected_result, expected_z, expected_s, expected_c, expected_o, description):
    ...
    @pytest.mark.parametrize("a, b, expected_result, expected_z, expected_s, expected_c, description", [
    ...
    @allure.description("Проверяет правильность выполнения операции вычитания и обновления флагов")
    def test_sub(self, alu_setup, a, b, expected_result, expected_z, expected_s, expected_c, description):
    ...
    @allure.description("Проверяет правильность выполнения операции умножения без переполнения")
    ...
    @allure.description("Проверяет правильность выполнения операции умножения с переполнением")
    ...
    @pytest.mark.parametrize("a, b, expected_z, expected_s, expected_c, description", [
    ...
    @allure.description("Проверяет правильность выполнения операции сравнения и установки флагов")
    def test_compare(self, alu_setup, a, b, expected_z, expected_s, expected_c, description):
    ...
    @pytest.mark.parametrize("a, b, expected_result, expected_z, expected_s, description", [
        (0xFF00FF00, 0x00FF00FF, 0x00000000, 1, 0, "AND с нулевым результатом"),
        (0xFFFFFFFF, 0x12345678, 0x12345678, 0, 0, "AND с сохранением второго операнда"),
    ...
    ], ids=["zero_result", "preserve_second", "with_zero", "opposite_bits", "alternating"])
    @allure.title("Логическое И: {description}")
    @allure.description("Проверяет правильность выполнения операции логического И")
    def test_logical_and(self, alu_setup, a, b, expected_result, expected_z, expected_s, description):
        """Тест логического И"""
        alu, registers, flags = alu_setup
    ...
    def test_logical_or(self, alu_setup, a, b, expected_result, expected_z, expected_s, description):
    ...
    @allure.description("Проверяет правильность выполнения операции логического исключающего ИЛИ")
    def test_logical_xor(self, alu_setup, a, b, expected_result, expected_z, expected_s, description):
    ...
    @pytest.mark.parametrize("a, expected_result, expected_z, expected_s, description", [
    ...
    @allure.description("Проверяет правильность выполнения операции логического НЕ (инверсии)")
    def test_logical_not(self, alu_setup, a, expected_result, expected_z, expected_s, description):
        """Тест логического НЕ"""
        alu, registers, flags = alu_setup
    ...
    @allure.description("Проверяет арифметический сдвиг вправо для положительного числа")
    ...
    @allure.description("Проверяет арифметический сдвиг вправо для отрицательного числа с сохранением знака")
    ...
    def test_operations_with_large_numbers(self, alu_setup):
        ...
        expected = ((0x1FFFFFFFF & 0xFFFFFFFF) + (0x200000000 & 0xFFFFFFFF)) & 0xFFFFFFFF
    ...
    def test_shift_large_count(self, alu_setup):
        ...
        result = alu.shift_left(0x12345678, 100)  # Эквивалентно сдвигу на 4 (100 & 0x1F)
